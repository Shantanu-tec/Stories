package com.example.stories.story.custom
...
constructor(
    ...
    private fun applyAttributes(attrs: AttributeSet?) {
        ...
        progressBarBackgroundColor = attrs.getColor(
            R.styleable.AndroidStories_progressBarBackgroundColor, resources.getColor(
                R.color.progressGray
            ))
        progressColor = attrs.getColor(
            R.styleable.AndroidStories_progressBarColor, resources.getColor(
                R.color.progressWhite
            ))
        storyDuration = attrs.getString(R.styleable.AndroidStories_storyDuration) ?: "3"
        loadingViewProgressColor = attrs.getColor(
            R.styleable.AndroidStories_loadingViewProgressColor, resources.getColor(
                R.color.progressWhite
            ))
        attrs.recycle()
    }
...
//        storyIndex = 1
    ...
    private fun addLoadingViews(storiesList: List<StoryItem>) {
        var idcounter = 1
        storiesList.forEachIndexed { index, story ->
    ...
    private fun onStoryPageCompleted() {
        if (::storiesListener.isInitialized) {
            storiesListener.onStoryPageEnd()
        }
    }
    ...
    private var loadListener:RequestListener<Drawable>? = object: RequestListener<Drawable> {
